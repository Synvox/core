...
export default function core<Context>(
  ...
  const read = async (
    ...
    const includeRelated = async (stmt: QueryBuilder) => {
      ...
      for (let includeTable of include) {
        ...
        if (isOne) {
          ...
          stmt.select(knex.raw(`array(${sql}) as ??`, [...bindings, ref.key]));
        }
      }
    ...
    const paginate = async (statement: QueryBuilder) => {
      ...
      return {
        ...
        data: await Promise.all(
          results.map(
            async (item: any) =>
              await processTableRow(queryParams, context, table, item, include)
          )
        ),
      };
    };
    ...
    if (many) {
      ...
      return await processTableRow(queryParams, context, table, data, include);
    }
  };
  ...
  const write = async (context: Context, table: Table<Context>, graph: any) => {
    ...
    const validateGraph = async (table: Table<Context>, graph: any) => {
      ...
      const placeholderFor = (table: Table<Context>) => {
        return postgresTypesToYupType(table.columns!.id.type).type === 'string'
          ? refPlaceholderUUID
    ...
    const updateGraph = async (
      ...
      const insert = async (
        ...
        let updatedRow = await stmt.where(`${table.alias}.id`, row.id).first();
  ...
  app.use(
    wrap(async (_req, _res, next) => {
      ...
      await Promise.all(
        tables.map(table =>
          initTable(table, knex, loadSchemaFromFile ? writeSchemaToFile : null)
        )
      );
          ...
          router.get(
            ...
            wrap(async (req, res) => {
              ...
              return {
                ...
                data: await table.getters[getterName].call(table, row, context),
              };
            })
          );
        }
